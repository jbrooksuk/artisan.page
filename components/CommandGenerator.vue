<template>
  <div
    ...
    <div v-for="(argument, key) in command.arguments" class="relative text-gray-400 focus-within:text-gray-600 px-4">
      <input v-model.trim="argument.value"
        ...
        :placeholder="argument.name" :tabindex="key" spellcheck="false" autocomplete="off" />
    </div>
    ...
    <div v-for="(option, key) in command.options"
      ...
      <template v-if="option.input_type === 'select'">
        ...
        <select v-model="option.value"
          ...
          <option v-for="option in option.input_options" :value="option" :key="option">
            {{ option }}
          </option>
        </select>
      </template>

      <template v-else-if="option.input_type === 'text'">
        <input id="text" v-model.trim="option.value"
          ...
          :placeholder="option.name" :tabindex="key" spellcheck="false" autocomplete="off" />
      </template>

      <div class="flex items-center h-5" v-else>
        <input :id="key" :tabindex="key" v-model.trim="option.value" type="checkbox"
...
php artisan {{ newCommand.trim() }}</pre>
        ...
        <div class="pr-8 flex-1 text-right">
          <button type="button"
            ...
            <svg v-if="copied" class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
